...
class BaseDataLoaderComponent(autoPyTorchTrainingComponent):
    ...
    def fit(self, X: Dict[str, Any], y: Any = None) -> torch.utils.data.DataLoader:
        ...
        train_dataset = datamanager.get_dataset(split_id=X['split_id'], repeat_id=X['repeat_id'], train=True)
        ...
        if X.get('val_indices', None) is not None:
            val_dataset = datamanager.get_dataset(split_id=X['split_id'], repeat_id=X['repeat_id'], train=False)
