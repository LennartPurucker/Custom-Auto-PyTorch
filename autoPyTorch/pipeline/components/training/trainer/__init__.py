...
class TrainerChoice(autoPyTorchChoice):
    ...
    def _fit(self, X: Dict[str, Any], y: Any = None, **kwargs: Any) -> 'TrainerChoice':
        ...
        if self.choice.use_stochastic_weight_averaging and self.choice.swa_updated:
            ...
            use_double = 'float64' in X['preprocessed_dtype']  # isinstance(X['preprocessed_dtype'], np.float64)
            self.logger.debug(f" Dtype of data : {X['preprocessed_dtype']}, use_double: {use_double}")
            # update batch norm statistics
            model = self.choice.swa_model.double() if use_double else self.choice.swa_model
            swa_utils.update_bn(loader=X['train_data_loader'], model=model)
            # change model
